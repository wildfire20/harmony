...
**Why:** Aggregate carry-forward invoices and repeated payment edits can otherwise double-count debt, reverse unrelated allocations, or make Admin and Parent balances disagree. Best-effort audits can leave permanent finance changes without required lineage, and clamped reversals can hide inconsistent balances. Current configuration can change after billing, and payment metadata can describe a different month from the invoice that actually received the allocation. Legacy invoice totals are authoritative amounts but do not by themselves prove a service category.

**How to apply:** Allocate under transaction locks, target exact allocation and successor identities, write required correction audits before commit, notify parents after commit, snapshot approved charges and discounts when invoices are issued, use only persisted descriptions for narrow legacy Tuition compatibility, create audited missing-service charges prospectively, expose reversal and review state, fail closed when history or balances are inconsistent, and test Admin/Parent/export equality across every payment channel.