...
import { Calendar, Clock, BookOpen, Save, ArrowLeft, AlertTriangle, Upload, FileText } from 'lucide-react';
...
const CreateTask = () => {
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      if (!allowedTypes.includes(file.type)) {
        setFileError('Invalid file type. Only PDF, Word, Excel, PowerPoint, Text, and Image files are allowed.');
        return;
      }
  ...
  // Create task mutation - Updated to handle file upload
  const createTaskMutation = useMutation(
    async (taskData) => {
      if (selectedFile) {
        ...
        // Append the file with the exact field name expected by multer
        ...
        return tasksAPI.createTaskWithFile(formData);
      } else {
  ...
  const handleSubmit = (e) => {
    ...
    // Don't include file in taskData to avoid duplicate fields in FormData
    // The file will be handled separately in the mutation
          ...
          <div>
            ...
            <div className="space-y-3">
              <div className="flex items-center space-x-4">
                <label
                  htmlFor="file_input"
                  className="flex items-center px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 cursor-pointer"
                >
              ...
              <p className="text-xs text-gray-500">
                Attach materials, worksheets, or reference documents for your assignment.
                Supported formats: PDF, DOC, DOCX, TXT, JPG, PNG, PPT, XLS. Max size: 10MB
              </p>
            </div>
          </div>
