...
import { AlertCircle, ChevronRight, Clock3, FileCheck2, KeyRound, Mail, RefreshCw, X } from 'lucide-react';
import api, { enrollmentsAPI } from '../../services/api';
...
const EnrollmentManagement = () => {
  ...
  return (
    <div className="p-6">
      <div className="mb-8"><h1 className="mb-2 text-2xl font-bold text-slate-900 dark:text-white">Admissions Applications</h1><p className="text-slate-600 dark:text-slate-400">A focused view of decisions, parent follow-ups and registration readiness.</p></div>
        ...
        <div className="modal-content max-h-[94vh] w-full max-w-5xl overflow-y-auto rounded-2xl border border-slate-200 bg-slate-50 shadow-2xl dark:border-slate-700 dark:bg-slate-900">
          ...
          <div className="grid gap-4 p-4 sm:p-6 lg:grid-cols-[1.35fr_0.85fr]">
            <div className="space-y-4">
              ...
              <Section eyebrow="Documents" title="Checklist status" icon={FileCheck2}><div className="grid gap-2 sm:grid-cols-2">{checklist.length ? checklist.map((item, index) => <div key={`${item.itemType}-${index}`} className="rounded-lg bg-slate-50 px-3 py-2.5 dark:bg-slate-900"><div className="flex items-center justify-between gap-2"><span className="text-sm text-slate-700 dark:text-slate-200">{CHECKLIST_LABELS[item.itemType] || item.itemType}</span><MiniBadge tone={item.status === 'RECEIVED' ? 'good' : item.status === 'MISSING' ? 'warn' : 'neutral'}>{CHECKLIST_STATES[item.status] || item.status}</MiniBadge></div><div className="mt-2 flex items-center gap-2"><select value={item.status} disabled={actionLoading || item.status === 'BRING_IN_PERSON'} onChange={(event) => updateChecklist(item.itemType, event.target.value)} className="min-h-8 flex-1 rounded-md border border-slate-300 px-2 py-1 text-xs dark:border-slate-600 dark:bg-slate-800"><option value={item.status}>{CHECKLIST_STATES[item.status] || item.status}</option>{ADMIN_CHECKLIST_STATES.filter(([value]) => value !== item.status).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>{item.receivedAt && <span className="text-[11px] text-slate-400">Received {formatDate(item.receivedAt)}</span>}</div></div>) : <p className="text-sm text-slate-500">No checklist items returned.</p>}</div><p className="mt-3 text-xs text-slate-400">Admin can set Missing, Received or Not applicable. Bring in person is displayed but not editable.</p></Section>
